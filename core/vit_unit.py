...
class Attention(nn.Module):
    def __init__(
        ...
        self.scale = qk_scale or head_dim ** -0.5   # for different len of head_dim, Q@K could be huge which is bad for softmax.
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 attn_head_dim=None, device=torch.device('cuda', index=0)):
        super().__init__()
        self.norm1 = norm_layer(dim, eps=1e-6).to(device)
        self.attn = Attention(
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2)   # B patch_num embed_dim
        return x
